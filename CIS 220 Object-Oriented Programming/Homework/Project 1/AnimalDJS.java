...
public class AnimalDJS
{
	...
	protected int winX, winY; //The current environmental boundaries
	
	//Constructor initializes instance data and creates an AnimalDJS object
	public AnimalDJS (int upperX, int upperY, int maxX, int maxY)
	{
		x = upperX;
		y = upperY;
		winX = maxX;
	...
	public void move(int upperBounds)
	{
		...
		if(energy >= .05)
		{
			...
			else //executes if the Animal moves on the y-axis
			{
				...
				if(direction == 0 && (y - radius >= upperBounds))
				{
					energy -= 0.05; //subtracts energy
					y -= radius; //steps up
				}
	...
	*/
	public void flee(AnimalDJS fauna, double power, int upperBounds)
	{
		int radius = (int)((double)diameter/2);
		energy += power;
		if(energy >= .05)
		{
			if(x - fauna.getX() < 0) //checks if this is moving left
			{
				...
				if(x - radius >= 0)
				{
					...
					energy -= .05; //applies energy change
				}
