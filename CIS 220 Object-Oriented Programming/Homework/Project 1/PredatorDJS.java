...
public class PredatorDJS extends AnimalDJS
{
	...
	public PredatorDJS(int upperX, int upperY, int maxX, int maxY)
	{
		super(upperX, upperY, maxX, maxY);
		color = Color.yellow;
		energy = 1.5;
	}
	...
	*/
	public void move(int upperBounds)
	{
		if(energy - .05 >= 0)
		{
			...
			if((generator.nextInt(4) == 0) && (x - radius >= 0) && (energy >= .05))
			{
			...
			else
			{
				...
				if((generator.nextInt(4) == 0) && (x + diameter + radius <= winX) &&
				(energy >= .05))
				{
			...
			if((generator.nextInt(4) == 0) && (y - radius >= upperBounds) && 
			(energy >= .05))
			{
			...
			else
			{
				...
				if((generator.nextInt(4) == 0) && (y + diameter + radius <= winY) &&
				(energy >= .05))
				{
		...
		else //sets dead Animal's energy to zero
		{
			if(energy <= 0)
			{
				energy = 0;
